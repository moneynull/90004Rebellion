...
public class Rebellion {
    ...
    public static Logger runSimulation(Params params) throws Exception {
        ...
        for(int i=0;i<nAgents;i++)
            agents[i] = new Agent(random.nextDouble(),random.nextDouble(), params.GOVERNMENT_LEGITIMACY);
        ...
        for (Cell[] row : world) {
            for (Cell cell : row) {
                ...
                if (cell.getOccupant() instanceof Cop)
                    single = "\u001B[44m" + " C ";
                else if (cell.getOccupant() instanceof Agent && ((Agent) cell.getOccupant()).isActive())
                    single = "\u001B[41m" + " A ";
                else if (cell.getOccupant() instanceof Agent && !((Agent) cell.getOccupant()).isActive())
                    single = "\u001B[43m" + " Q ";
